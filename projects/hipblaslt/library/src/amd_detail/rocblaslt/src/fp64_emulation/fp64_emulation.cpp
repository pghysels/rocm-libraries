...
#pragma unroll
    ...
    __global__ static void scale_A_N_kernel(const double* __restrict__ A,
                                            ...
                                            unsigned t_start)
    {
        static constexpr int TILE_K = OZ2_SCALE_TILE_K;
        static constexpr int TILE_M = OZ2_SCALE_SHMEM_TILE_M;
        __shared__ double    shmem[TILE_K][TILE_M + 1];
        __shared__ int16_t   s_sft[TILE_M];
...
#pragma unroll
            ...
            {
                const unsigned tidx = t_start + t_local;
                const double   r    = fma(neg_mod(tidx), rint(ival * inv_mod(tidx)), ival);
                const float    rf   = static_cast<float>(r);
                const float    rf2
                    = fmaf(rintf(rf * inv_mod_f(tidx)), static_cast<float>(neg_mod(tidx)), rf);
                               ...
                               hipStream_t   stream)
    {
        ...
        if(tA)
            ...
            hipLaunchKernelGGL((scale_A_N_kernel<TC>),
                               ...
                               dim3(OZ2_SCALE_TILE_K * OZ2_SCALE_SHMEM_TILE_M),
                               0,
                               stream,
                               A,
                               m,
