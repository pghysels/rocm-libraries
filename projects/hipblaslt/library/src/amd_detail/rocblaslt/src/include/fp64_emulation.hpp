...
/* Returns the number of INT8 GEMMs (moduli) to use, in the range [2, 14].
 ...
 * Default (env var absent or 0): 14 moduli (~110 bits of CRT capacity).
 * Notable values: 55 bits → 7 GEMMs, 79 bits → 10 GEMMs (design doc §2.5). */
...
struct Fp64EmulationSettings {
    unsigned int num_moduli;      /* 2..14; 0 = derive from env var          */
